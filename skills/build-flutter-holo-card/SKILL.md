...
3. Confirm that this variant is wanted:
   ...
   - model-generated smooth contour-only lines for the accepted foreground plus derived bloom, with no invented or decorative internal detail;
...
4. Generate one contour-only line transformation from the accepted `foreground.png`. Trace only source-visible silhouettes and source-visible overlap or separation contours. Include the accepted subject, subject-linked elements, typography, symbols, panels, logos, and frame, but never invent lines, reconstruct hidden shapes, or add facial, anatomical, hair, fur, fabric, surface, shading, hatching, highlight, or texture details. Require thin, smooth white antialiased lines on genuine transparency. Preserve the complete canvas and do not add, fill, reconstruct, or rearrange content. Normalize the model output with `prepare_generated_lineart.py`; it also removes an accidentally baked checkerboard without re-detecting source edges.
...
- contour-only grayscale `character_contour.png`, or a neutral black compatibility map;
...
1. Inspect resources individually:
   ...
   - enabled structure is smooth black/white contour-only line art, not a colored packed map and not a detail sketch;
   - disabled structure and bloom are both neutral black;
   - bloom lives in a separate map.
2. When contour is enabled, inspect the structure overlay at full canvas. Reject displaced contours, invented internal detail, effects that do not belong to the subject composition, or independently normalized layers.
...
- Stop if structure adds anything beyond source-visible silhouettes and overlap or separation contours. Regenerate it or disable contour; never accept invented internal details.
